-- ============================================
-- 033: Fix Bonus Pool Calculation & Cashback Rate Cap
-- ============================================
-- Changes:
-- 1. Cashback rate cap: first (pkg*5) boxes = 50%, next (pkg*5) = 30%, after (pkg*10) = 0%
-- 2. Bonus pool contribution = (order_total - total_cashback_paid) * 30%
--    (deduct direct cashback + regional bonuses before calculating pool)
-- 3. Move bonus pool logic INTO process_order_cashback; drop standalone trigger
-- 4. Always update total_boxes_processed even when cashback blocked/exhausted

-- ============================================
-- Update cashback rate function with proper cap
-- ============================================
CREATE OR REPLACE FUNCTION get_partner_cashback_rate(p_partner_id VARCHAR, p_current_box_count INTEGER)
RETURNS NUMERIC AS $$
DECLARE
  v_packages INTEGER;
  v_fifty_limit INTEGER;
  v_thirty_limit INTEGER;
BEGIN
  SELECT COALESCE(packages_purchased, 1) INTO v_packages
  FROM partners WHERE id = p_partner_id;

  v_fifty_limit  := v_packages * 5;   -- e.g. 1 pkg → first 5 boxes
  v_thirty_limit := v_packages * 10;  -- e.g. 1 pkg → next 5 boxes (6-10)

  IF p_current_box_count < v_fifty_limit THEN
    RETURN 0.50;
  ELSIF p_current_box_count < v_thirty_limit THEN
    RETURN 0.30;
  ELSE
    RETURN 0;  -- quota exhausted, no more cashback
  END IF;
END;
$$ LANGUAGE plpgsql;
...
-- Now includes bonus pool contribution at the end
-- ============================================
        ...
        IF v_same_level_cashback > 0 THEN
          ...
          ELSE
            ...
            VALUES (gen_random_uuid()::text, v_parent_partner_id, 'deduct', -v_same_level_ly_required, p_order_id, 'cashback',
                    'Regional bonus LY deduction (Gen ' || v_gen || ')', NOW());
            ...
            VALUES (gen_random_uuid()::text, v_parent_partner_id, 'cashback', v_same_level_cashback, p_order_id, 'order', 'completed',
                    'Regional bonus (Gen ' || v_gen || ', 10% of direct cashback)', NOW());
...
-- ============================================
-- Drop standalone bonus pool trigger (now handled inside process_order_cashback)
-- Keep the function for reference but disable the trigger
-- ============================================
DROP TRIGGER IF EXISTS trg_order_bonus_pool ON orders;

-- ============================================
-- Ensure the delivered cashback trigger still works
-- ============================================
